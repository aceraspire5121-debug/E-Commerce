...
const savingProduct= async (data)=>{
  ...
  const token=localStorage.getItem("CommerceToken")
     const res=await fetch(`http://127.0.0.1:3000/api/products/${id}`,{ // isme mujhe files+text data dono bhejne hai isliye formData ka use karna padega barna mai direct likh skta tha body:json.stringfy(data), par isse files nhi jayengi isliye formdata ka use karunga
      method:"PUT",
        headers: {
